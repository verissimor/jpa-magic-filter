...
object R2dbcSqlWriter {
  ...
  fun writeSql(parsedFields: List<ParsedField>, tableAlias: String? = null): SqlBinder {
    val alias = when {
      tableAlias == null -> ""
    ...
    val sqlWhere = parsedFields.mapIndexed { index: Int, field: ParsedField ->
  ...
  private fun resolveFieldValue(parsed: ParsedField) = when (parsed.getFieldType()) {
  ...
  private fun resolveListFieldValue(parsed: ParsedField) = when (parsed.getFieldType()) {
