...
object R2dbcPredicateParser {
  ...
  fun parsePredicates(parsedField: ParsedField, dbFeatures: DbFeatures): Criteria = when (parsedField.filterOperator) {
  ...
  private fun parseLike(parsedField: ParsedField, dbFeatures: DbFeatures) = when (dbFeatures) {
    POSTGRES -> where("unaccent(lower(${parsedField.resolvedFieldName}))").like(
      "%${parsedField.getString().lowercase().unaccent()}%"
    )
    NONE -> where(parsedField.resolvedFieldName).like("%${parsedField.getString().lowercase()}%")
  }

  private fun parseLikeExp(parsedField: ParsedField, dbFeatures: DbFeatures) = when (dbFeatures) {
    POSTGRES -> where("unaccent(lower(${parsedField.resolvedFieldName}))").like(
      parsedField.getString().lowercase().unaccent()
    )
    NONE -> where(parsedField.resolvedFieldName).like(parsedField.getString().lowercase())
  ...
  private fun parseNotLike(parsedField: ParsedField, dbFeatures: DbFeatures) = when (dbFeatures) {
    POSTGRES -> where("unaccent(lower(${parsedField.resolvedFieldName}))").notLike(
      "%${parsedField.getString().lowercase().unaccent()}%"
    )
    NONE -> where(parsedField.resolvedFieldName).notLike("%${parsedField.getString().lowercase()}%")
  }

  private fun parseNotLikeExp(parsedField: ParsedField, dbFeatures: DbFeatures) = when (dbFeatures) {
    POSTGRES -> where("unaccent(lower(${parsedField.resolvedFieldName}))").notLike(
      parsedField.getString().lowercase().unaccent()
    )
    NONE -> where(parsedField.resolvedFieldName).notLike(parsedField.getString().lowercase())
  ...
  private fun parseEqual(parsedField: ParsedField) = when (parsedField.getFieldType()) {
  ...
  private fun parseNotEqual(parsedField: ParsedField) = when (parsedField.getFieldType()) {
  ...
  private fun parseGreaterThan(parsedField: ParsedField) = when (parsedField.getFieldType()) {
  ...
  private fun parseGreaterThanEqual(parsedField: ParsedField) = when (parsedField.getFieldType()) {
  ...
  private fun parseLessThan(parsedField: ParsedField) = when (parsedField.getFieldType()) {
  ...
  private fun parseLessThanEqual(parsedField: ParsedField) = when (parsedField.getFieldType()) {
