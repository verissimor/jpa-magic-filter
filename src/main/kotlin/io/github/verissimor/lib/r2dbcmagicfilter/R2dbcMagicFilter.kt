...
class R2dbcMagicFilter(
  ...
  private val parameters: MutableMap<String, List<String>?> = parameterMap.keys.associateWith {
  ...
  fun toCriteria(clazz: Class<*>, dbFeatures: DbFeatures = NONE): Criteria {
    ...
    val criteriaGroups = groups.map { group ->
      ...
      when (fieldsOfGroup.size) {
        ...
        else -> fieldsOfGroup.fold(Criteria.empty()) { acc, field ->
    ...
    val criteria: Criteria = when (criteriaGroups.size) {
      1 -> criteriaGroups.first()
      else -> criteriaGroups.fold(Criteria.empty()) { acc, criteria ->
        when (getSearchType()) {
  ...
  fun getSearchType(): CombineOperator = when (parameters[SEARCH_TYPE_PRM]?.firstOrNull()) {
  ...
  fun toSqlBinder(clazz: Class<*>, tableAlias: String? = null, startStr: String = " AND "): SqlBinder? {
    val parseFields = toParsedFields(clazz)
    val groups = parseFields.map { it.group }.distinct().sorted()
    ...
    // first reduce/fold criteria in the group
    val groupSqlList = groups.map { group ->
      val fieldsOfGroup = parseFields.filter { it.group == group }
      R2dbcSqlWriter.writeSql(fieldsOfGroup, tableAlias)
    }

    val startBinder = SqlBinder("", emptyMap())
    val foldedBinder = groupSqlList.foldIndexed(startBinder) { idx, acc, sql ->
      val newParams = acc.params + sql.params
      val combineOperator = when {
        idx == 0 -> ""
